...
- **A+ Polish**: Dry-run simulated anchor no longer updates placeholder proof rows to `status='anchored'` or sets `casper_anchor_hash`. Placeholder proofs stay `status='pending'` with no persisted anchor hash. The simulated anchorHash is returned only in the API response. All 5 gates green (631 tests). Ready for Levi A+ re-audit.
